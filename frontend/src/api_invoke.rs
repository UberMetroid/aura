...
pub fn spawn_invoke_generation(
    ...
    web_sys::console::log_1(&format!("InvokeAI: Starting image generation for query: {}", query).into());
    ...
    leptos::spawn_local(async move {
        match generate_invoke_image(&query).await {
            Ok(item_id) => {
                web_sys::console::log_1(&format!("InvokeAI: Enqueued successfully. Received item_id = {}", item_id).into());
                poll_status(item_id, set_image, set_loading, set_error);
            }
            Err(e) => {
                web_sys::console::log_1(&format!("InvokeAI: Failed to start generation: {}", e).into());
...
fn poll_status(
    ...
    web_sys::console::log_1(&format!("InvokeAI: Scheduling next poll for item_id = {}", item_id).into());
    gloo_timers::callback::Timeout::new(1000, move || {
        leptos::spawn_local(async move {
            web_sys::console::log_1(&format!("InvokeAI: Querying status for item_id = {}", item_id).into());
            match get_invoke_image_status(item_id).await {
                Ok(status_resp) => {
                    web_sys::console::log_1(&format!("InvokeAI: Status response = {}", status_resp.status).into());
                    if status_resp.status == "completed" {
                        if let Some(image_name) = status_resp.image_name {
                            let url = format!("/api/invoke-image-file/{}", image_name);
                            web_sys::console::log_1(&format!("InvokeAI: Generation completed successfully. Image URL = {}", url).into());
                            set_image.set(Some(url));
                        } else {
                            web_sys::console::log_1(&"InvokeAI: Generation completed but image_name is missing!".into());
                            set_error.set(Some("Completed but no image returned".to_string()));
                        }
                        set_loading.set(false);
                    } else if status_resp.status == "failed" {
                        let err_msg = status_resp.error.unwrap_or_else(|| "Generation failed".to_string());
                        web_sys::console::log_1(&format!("InvokeAI: Generation failed. Error = {}", err_msg).into());
                        set_error.set(Some(err_msg));
                        set_loading.set(false);
                    } else {
                        web_sys::console::log_1(&"InvokeAI: Generation still running. Rescheduling poll...".into());
                        poll_status(item_id, set_image, set_loading, set_error);
                    }
                }
                Err(e) => {
                    web_sys::console::log_1(&format!("InvokeAI: Poll query failed with network/API error = {}", e).into());
                    set_error.set(Some(e));
                    set_loading.set(false);
                }
            }
...
pub fn strip_speaker_labels(mut s: &str) -> String {
    loop {
        ...
        let trimmed = s.trim_start_matches(|c| c == ' ' || c == '*' || c == '_' || c == ':' || c == '-' || c == '\n' || c == '\r');
        for prefix in &["ai assistant", "ai", "overview", "answer", "assistant"] {
            if trimmed.to_lowercase().starts_with(prefix) {
                ...
                if next_char.is_none() || next_char.map_or(true, |c| !c.is_alphanumeric()) {
                    let mut rest = &trimmed[prefix.len()..];
                    rest = rest.trim_start_matches(|c| c == ' ' || c == '*' || c == '_' || c == ':' || c == '-' || c == '\n' || c == '\r');
                    s = rest;
                    changed = true;
                    break;
                }
